...
object LocationConfig {
    ...
    /** Crash-recovery: true = текущая пауза поставлена автопаузой (не пользователем).
     *  После рестарта сервис продолжает авто-резюмить только такую паузу —
    ...
    // ── Автопауза ────────────────────────────────────────────────────────────────
    // Гистерезис: порог входа в паузу ниже порога выхода — GPS-дрейф на месте
    // (кажущаяся скорость 0.5–1.0 м/с) не даёт ложных резюмов.

    /** Скорость (м/с) ниже которой движение считается остановкой (≈1.8 км/ч). */
    const val AUTOPAUSE_PAUSE_SPEED_MPS  = 0.5f

    /** Скорость (м/с) для автоматического снятия паузы (≈3.6 км/ч). */
    const val AUTOPAUSE_RESUME_SPEED_MPS = 1.0f

    /** Сколько непрерывной «остановки» нужно для срабатывания автопаузы (мс). */
    const val AUTOPAUSE_MIN_STILL_MS     = 5_000L

    /** Сколько последовательных «движущихся» точек нужно для авто-резюма. */
    const val AUTOPAUSE_RESUME_POINTS    = 2

    /** Первые секунды после старта тренировки автопауза не срабатывает —
     *  пользователь ещё стоит на старте, мгновенная пауза сбивает с толку. */
    const val AUTOPAUSE_WARMUP_MS        = 10_000L
