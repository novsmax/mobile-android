...
class RegisterViewModel @Inject constructor(
    ...
    // Список разрешённых доменов почты (149-ФЗ). Загружается в init из
    // репозитория (сервер → кэш → зашитый fallback, BR-4); пока корутина init
    // не завершилась — set пуст и локальная проверка пропускается, домен всё
    // равно проверит RegisterUseCase перед запросом к серверу.
