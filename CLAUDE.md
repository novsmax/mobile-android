...
по зажатию 3 сек с заполнением, защита от случайного нажатия).
Хранение — `SettingsStorage` (DataStore Preferences). Там же служебный флаг
`workoutCoachmarkShown` (не в UI) — одноразовый onboarding-coachmark при первом
входе в активную тренировку (WorkoutStartScreen: `WorkoutCoachmark`); плюс на
короткий тап «Завершить» в hold-режиме выезжает хинт «Удерживайте 3 сек»
(`FinishHoldHint`).
