...
- `@Composable fun SettingsScreen(settings: AppSettings, onBack: () -> Unit, onAutopauseChanged, onVoiceCuesChanged, onVoiceCueIntervalChanged, onKeepScreenOnChanged, onOpenSensors: () -> Unit)` — три `SwitchRow` («Автопауза», «Голосовые подсказки», «Не гасить экран») + `IntervalSelectorRow` (частота подсказок 1/2/5 км) + `VolumeSliderRow` (громкость подсказок; оба видны только при включённых подсказках) + секция «Датчики» с `NavigationRow` «Пульсометр» (подзаголовок — имя/адрес активного датчика, иначе «Датчиков: N» при непустом списке, иначе «Не настроен»; клик → `onOpenSensors`).
