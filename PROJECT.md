...
- `@Composable fun AppNavGraph(navController: NavHostController = rememberNavController(), startDestination: String = Screen.Login.route, onLogout: () -> Unit = {}, sessionExpired: StateFlow<Boolean> = MutableStateFlow(false))`.
  ...
  - `composable(Screen.Home.route)` — `WorkoutHomeScreen` с `onLogout` (вызывает `onLogout()` из параметров графа и очищает весь бэкстек `popUpTo(navController.graph.id) { inclusive = true }`), `onNavigateToProfile`, `onNavigateToSettings` и `onOpenSensors` (тап по HR-бейджу на экране тренировки → `Screen.Sensors`).
...
- `@Composable fun SensorsScreen(state: SensorsUiState, onBack, onPermissionsGranted, onPermissionsDenied, onScanClick, onDeviceClick, onConnectSavedClick, onForgetClick)` — вызывает `BluetoothPermissionHandler`; карточка сохранённого датчика (`SavedDeviceCard`: имя, статус + живой BPM при CONNECTED, кнопки «Подключить»/«Забыть датчик») или подсказка «датчик не настроен»; `PrimaryButton` «Поиск датчиков» (disabled без разрешений/во время скана, спиннер при скане); список `ScanResultRow` (имя/адрес/rssi, клик → подключение); подсказки при отказе в разрешениях и пустом результате скана (`hasScanned`-гейт).
...
- `@Composable fun WorkoutHomeScreen(onLogout, onNavigateToProfile, onNavigateToSettings, onOpenSensors)` — хоистит `WorkoutStartViewModel` на верхнем уровне (не внутри `when`), чтобы оверлей итогов тренировки переживал переключение вкладок; `onOpenSensors` пробрасывается в `WorkoutStartScreen` (тап по HR-бейджу). Использует `Scaffold` с `SmartTrackerBottomBar`; рендерит `WorkoutStartScreen`, `TrainingHistoryScreen` или `MenuScreen` в зависимости от выбранной вкладки.
...
- **HR-бейдж** — под GPS-бейджем (top-end карты, `padding(top = 48.dp)`), виден только при `hrmConfigured` и вне оверлея; статусный: круглый 32dp, иконка `Icons.Filled.Favorite`, фон `ColorGpsActive`/`ColorGpsInactive` по `hrmConnected`. Значение пульса на бейдже НЕ показывается — оно в StatItem «Пульс» (дублирование на карте шумит). Тап → **отложенная** навигация на экран «Датчики» (нюанс 36): `sensorsNavPending = true` → `suppressLocationDot` гасит LocationComponent в `MapViewComposable` → `delay(HRM_NAV_DELAY_MS = 350 мс)` → `onOpenSensors()`; немедленный navigate ронял процесс гонкой аниматоров MapLibre со Style, а пауза меньше ~250 мс не спасает — уже запущенный accuracy-AnimatorSet неотменяем и дотикивает сам. Лямбда пробрасывается `AppNavGraph` → `WorkoutHomeScreen` → `WorkoutStartScreen`.
