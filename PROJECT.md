...
по состоянию на 2026-07-22, коммит `c9c8347`, ветка `main`.
...
- `@Composable private fun WorkoutCoachmark(finishConfirmationHold, finishButtonBounds, onDismiss)` + `CoachmarkTip(text)` — одноразовый onboarding при первом входе в активную фазу (`isWorkoutStarted && !coachmarkShown && !overlayVisible && !isFullscreen`). **Spotlight**: скрим `Black α0.62` с «дыркой» точно над кнопкой «Завершить» — она светится сквозь затемнение (`Canvas` + `drawRoundRect(Color.Transparent, blendMode = BlendMode.Clear)` в offscreen-слое `graphicsLayer{ compositingStrategy = Offscreen }`), обведена `ColorSecondary`, к ней ведёт треугольник-стрелка; карточка советов (удержание Завершить, кнопки на паузе, автопауза в настройках) + «Понятно» позиционируется НАД кнопкой. Bounds «Завершить» приходят из `PausableFinishRow` (`onGloballyPositioned`/`boundsInWindow`) → hoisted `finishButtonBounds`; оверлей меряет свою `positionInWindow`/`size` для перевода в локальные координаты. Тап по скриму или «Понятно» → `onCoachmarkDismissed` (персист `setWorkoutCoachmarkShown`). Корневой `Column` экрана обёрнут в `Box(fillMaxSize)` для оверлея.
