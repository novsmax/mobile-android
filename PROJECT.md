...
- Разрешения: `INTERNET`, `ACCESS_FINE_LOCATION`/`ACCESS_COARSE_LOCATION`,
  ...
  `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`, `CAMERA`; Bluetooth (пульсометры):
  legacy `BLUETOOTH`/`BLUETOOTH_ADMIN` (`maxSdkVersion="30"`) + runtime
...
- `LocationTrackingService` — `foregroundServiceType="location|connectedDevice"`, `stopWithTask="false"`
  (сервис не убивается при смахивании приложения из Recents на Samsung/Xiaomi и т.п.);
  бит `connectedDevice` в `startForeground` подставляется условно — только при выданном
  BLUETOOTH_CONNECT (нюанс 35).
...
│   ├── hrm/              BLE-пульсометр: HrmManager (+Impl, singleton-владелец соединения)
│   │   ├── model/        HrmScanResult, HrmSample, HrmConnectionState
│   │   └── sensor/       HeartRateSensor, BleHeartRateSensor, HeartRateSensorFactory, HrmParser
│   ├── local/            TokenStorage, RoleConfigStorage, UserProfileCache, IconCacheManager
│   │   └── db/           Room v9: entities, DAO, мапперы
...
│   ├── menu/             MenuScreen + profile/ + settings/ + sensors/
...
- `SavedHrmDevice` (data class) — `address: String` (MAC, ключ уникальности и цель подключения), `name: String?` (для отображения).
...
- `SettingsStorageImpl` (`@Singleton`, `@Inject constructor(@ApplicationContext Context)`):
  - `Keys` (private object) — `booleanPreferencesKey`/`intPreferencesKey` для четырёх настроек + `HRM_DEVICES` (JSON-список) и `HRM_ACTIVE_ADDRESS`; legacy-ключи `LEGACY_HRM_DEVICE_ADDRESS`/`NAME` только для миграции;
  - `override val settings: Flow<AppSettings>` — `data.catch { IOException → emit(emptyPreferences()) }.map { … }`: повреждённый файл деградирует к дефолтам, прочие исключения пробрасываются (ошибки программирования должны попадать в crash-репорты); `hrmDevices` через `readHrmDevices` (мягкая миграция legacy-одиночного датчика → список из одного);
  - сеттеры — `context.settingsDataStore.edit { … }`; `setVoiceCueIntervalKm` валидирует значение; `addHrmDevice`/`removeHrmDevice`/`setActiveHrmDevice` работают со списком через `writeHrmDevices` (материализует JSON-формат + удаляет legacy-ключи).
- Internal pure-функции `encodeHrmDevices(list): String` / `decodeHrmDevices(json): List<SavedHrmDevice>` — Gson-сериализация списка (битый JSON → пустой список, не краш). Покрыты `HrmDevicesCodecTest`.
...
Особенности: комментарий в файле фиксирует историю схемы — schema v2 добавила `bearing` и `externalId`, schema v3 добавила `calories`, schema v9 добавила `heartRate`.
...
- `SmartTrackerDatabase` (`@Database(entities = [GpsPointEntity::class, ActivityTypeEntity::class, PendingFinishEntity::class, METActivityEntity::class, MetZoneEntity::class], version = 9, exportSchema = false) abstract class : RoomDatabase()`.
  ...
  - `companion object` содержит объекты миграций:
    ...
    - `MIGRATION_7_8` (`Migration(7, 8)`) — создаёт две новые таблицы одним SQL-блоком каждая:
      ...
      ```
    - `MIGRATION_8_9` (`Migration(8, 9)`) — `ALTER TABLE gps_points ADD COLUMN heartRate INTEGER` (пульс с BLE-пульсометра на момент записи точки).
...
- `@GET("auth/allowed-email-domains") suspend fun getAllowedEmailDomains(): AllowedEmailDomainsResponseDto` — серверный список почтовых доменов для регистрации (BR-4, 149-ФЗ); публичный, единый источник с серверной проверкой в `/auth/register`.
...
#### `data/remote/dto/AllowedEmailDomainsResponseDto.kt`
DTO ответа `GET /auth/allowed-email-domains` (BR-4, 149-ФЗ).
- `AllowedEmailDomainsResponseDto` (data class): `domains: List<String>?` — имя поля совпадает с JSON, `@SerializedName` не нужен; nullable защитно — репозиторий трактует null/пустой список как сбой и уходит на fallback.
...
- `GpsPointDto` (data class): `recordedAt: String` (`@SerializedName("recorded_at")`, ISO 8601 UTC), `latitude: Double`, `longitude: Double`, `accuracy: Float? = null`, `altitude: Double? = null`, `speed: Float? = null`, `calories: Double? = null` (расход калорий за интервал до точки; `null`, если профиль пользователя — вес/рост/возраст — ещё не заполнен), `heartRate: Int? = null` (`@SerializedName("heart_rate")`, пульс с BLE-датчика; Gson не сериализует null → точки без пульса уходят без поля; серверная поддержка — BR-16, до неё поле игнорируется бэкендом).
...
Сетевая реализация `AllowedEmailDomainsRepository` (BR-4, 149-ФЗ): `GET /auth/allowed-email-domains` с кэшем и офлайн-fallback.
- `internal val Context.allowedEmailDomainsDataStore by preferencesDataStore(name = "allowed_email_domains")` — DataStore-файл кэша (паттерн `SettingsStorageImpl`); в репозиторий попадает через `@Named("allowedEmailDomains")`-провайдер AuthModule — ради подмены в JVM-тестах.
- `AllowedEmailDomainsRepositoryImpl` (`@Singleton class`, `@Inject constructor(api: AuthApiService, @Named("allowedEmailDomains") dataStore: DataStore<Preferences>)`).
  - `override suspend fun getAllowedDomains(): Set<String>` — цепочка источников: (1) in-memory кэш успешного сетевого ответа (живёт до конца процесса; вызовов мало — init RegisterViewModel + сабмит, TTL не нужен); (2) сеть — список нормализуется (trim + lowercase) и персистится; (3) DataStore — последний успешный серверный ответ (офлайн-старт видит актуальный список, даже если APK-хардкод устарел); (4) `HARDCODED_RUSSIAN_DOMAINS`. Под `Mutex` с double-check — параллельные вызовы не дублируют запрос. Неудача сети в память не мемоизируется — следующий вызов пробует снова.
  - `private suspend fun fetchFromServer(): Set<String>?` — null при любом сбое; пустой/отсутствующий массив тоже сбой (пустой список заблокировал бы все регистрации). `CancellationException` перебрасывается.
  - `private suspend fun persist(...)` / `readPersisted(): Set<String>?` — запись/чтение `stringSetPreferencesKey("domains")`, обе в `runCatching` (контракт «не бросает»).
...
Особенности: канонический источник списка — сервер (`app/core/email_domains.py` бэка, единый для проверки и выдачи); хардкод должен зеркально соответствовать ему на момент сборки APK. Зеркальная проверка на бэкенде реализована (BR-2) — клиентская валидация остаётся UX-слоем. Тест: `AllowedEmailDomainsRepositoryImplTest` (DataStore на временном файле через `PreferenceDataStoreFactory`, без Robolectric).
...
- `SaveTrainingWorker` (`@HiltWorker`, class, `@AssistedInject constructor`, наследует `CoroutineWorker`) — assisted-параметры `Context`, `WorkerParameters`; DI-зависимости: `PendingFinishDao`, `WorkoutRepository`, `LocationRepository` (чистка GPS-точек закрытой тренировки из Room при успехе и при `TrainingAlreadyClosedException`).
  - `override suspend fun doWork(): Result` — читает `trainingId` из inputData (fallback на `resolvedId` от `SyncGpsPointsWorker.KEY_RESOLVED_TRAINING_ID`, если офлайн-тренировка была перерегистрирована с localUUID на serverUUID). Если `runAttemptCount >= MAX_ATTEMPTS` (5) — удаляет запись(и) из `PendingFinishDao` и возвращает `Result.failure()` (не блокирует WorkManager вечно). Иначе ищет конкретную запись по `resolvedId` (или fallback на `getAll()`, если resolvedId устарел после re-key). Для каждой найденной записи вызывает `workoutRepository.saveTraining(...)`, передавая nullable `totalDistanceMeters/totalKilocalories` как есть (BR-7 закрыт бэком: null-поля валидны, Gson дропает их из тела; дистанцию сервер считает из трека сам, `kilocalories=null` = «нет данных» — честнее фейкового 0; старый `?: 0.0`-фолбэк снят 21.07.2026). При успехе — удаляет запись из очереди; при `TrainingAlreadyClosedException` — тоже удаляет (ретрай бессмысленен); при прочих ошибках — не удаляет, флаг `hasTransientFailures=true`. Возвращает `Result.retry()` при наличии транзиентных ошибок, иначе `Result.success()`.
...
- `LocationTrackingService` (`@AndroidEntryPoint`, class, наследует `Service`) — инжектируемые поля: `LocationRepository`, `WorkoutRepository`, `OfflineMapManager`, `SettingsStorage`, `HrmManager` (пульсометр: автоподключение, пульс в точки, разрыв в onDestroy).
  ...
  - `private fun connectHrmIfConfigured()` — автоподключение к активному пульсометру: адрес из `settingsStorage.settings.first().autoConnectAddress()`, гейт `hasBluetoothConnectPermission()`; вызывается из ветки `EXTRA_TRANSITION_TO_WORKOUT`, обычного старта тренировки и START_STICKY-рестарта; в discovery НЕ вызывается. Дальше `HrmManager` сам удерживает соединение.
  - `private fun foregroundServiceTypes(): Int` — маска для `startForeground` (API 29+): `FOREGROUND_SERVICE_TYPE_LOCATION`, к которой OR-ится `FOREGROUND_SERVICE_TYPE_CONNECTED_DEVICE` ТОЛЬКО при выданном BLUETOOTH_CONNECT — на API 34+ тип без выполненного prerequisite кидает `SecurityException` (нюанс 35).
  - `private fun hasBluetoothConnectPermission(): Boolean` — API 31+ runtime-проверка BLUETOOTH_CONNECT; ниже — `true` (legacy BLUETOOTH выдан при установке).
  ...
  - `override fun onDestroy()` — останавливает трекер и таймеры, рвёт соединение с пульсометром (`hrmManager.disconnect()` — остановка бесконечных реконнектов), финально флашит буфер и синхронизирует (`syncUnsentPoints`) — либо для discovery удаляет накопленные точки; эмитит `_finishSyncFlow`; снимает foreground-статус, очищает crash-recovery prefs, освобождает WakeLock.
  ...
  - `companion object` — все Intent extras/actions, request codes, `finishSyncFlow` (`SharedFlow<Unit>` с `replay=1` — сигнал ViewModel'у, что финальный sync завершён и можно закрывать тренировку), `recordingStateFlow` (`replay=0` — только живые события паузы/резюма; `extraBufferCapacity=16` + Log.w при переполнении в `tryEmit` — потеря pause-события теряла бы gap-индекс, и live-дистанция навсегда посчитала бы «телепорт» через паузу), `fun setRecording(context, recording: Boolean)` — отправляет команду в работающий сервис.
...
- `OfflineMapManager` (`@Singleton`, class, `@Inject constructor()`) — зависимость: `@ApplicationContext Context` (помечена `@Suppress("unused")`, фактически не используется методами).
  ...
  - `companion object` — `STYLE_JSON`: inline MapLibre style spec v8 с raster-источником `tile.openstreetmap.org/{z}/{x}/{y}.png` (id источника/слоя `osm`; дипломный `tile.gottland.ru` выведен 21.07.2026), `tileSize=256`, `maxzoom=19`, атрибуция OSM в HTML-формате `<a href="...">`.

Особенности: раньше (при использовании vector-стиля OpenFreeMap) `MapLibre OfflineManager` умел скачивать pyramid-регионы по URL стиля; после перехода на raster XYZ стиль собирается inline JSON, а `OfflineTilePyramidRegionDefinition` требует URL — поэтому офлайн-предзагрузка отключена, но авто-кэш MapLibre (~100 МБ, LRU) продолжает работать прозрачно. **OSMF Tile Usage Policy**: обязателен идентифицирующий User-Agent (интерцептор в `AuthModule.provideOkHttpClient`; MapLibre ходит через общий клиент — `HttpRequestUtil.setOkHttpClient`), bulk-предзагрузка публичных тайлов запрещена (стаб обязан оставаться no-op), при заметном трафике — переезд на свой рендер/коммерческого провайдера (тех-долг в CLAUDE.md TODO). Атрибуция OSM обязательна по лицензии ODbL и не может быть отключена (в отличие от логотипа MapLibre).
...
Подсистема внешних BLE-пульсометров (GATT Heart Rate Service 0x180D). Архитектура зеркалит `data/location`: транспорт-агностичный интерфейс + фабрика (точка расширения для ANT+) + singleton-менеджер, владеющий соединением.

#### `data/hrm/model/HrmModels.kt`
Транспорт-агностичные модели подсистемы HRM (без типов Android Bluetooth API).
- `HrmScanResult` (data class) — `address: String` (MAC, ключ дедупликации), `name: String?` (из advertising-пакета, может отсутствовать), `rssi: Int` (dBm, для сортировки списка).
- `HrmSample` (data class) — `bpm: Int`, `timestampMs: Long` (для проверки свежести: протухший сэмпл после обрыва не пишется в точки).
- `HrmConnectionState` (enum) — `DISCONNECTED`, `CONNECTING`, `CONNECTED`, `RECONNECTING` (обрыв, менеджер бесконечно ретраит), `BLUETOOTH_OFF` (адаптер выключен, ретраи приостановлены).

#### `data/hrm/sensor/HrmParser.kt`
Чистый парсер характеристики Heart Rate Measurement (0x2A37, Bluetooth SIG).
- `HrmParser` (object) — `fun parseHeartRateMeasurement(value: ByteArray): Int?` — flags-байт: бит 0 = 0 → UINT8 в байте 1, бит 0 = 1 → UINT16 little-endian в байтах 1-2; malformed payload → null (сэмпл отбрасывается, соединение не рвётся). Биты sensor contact/Energy Expended/RR-интервалов идут ПОСЛЕ значения пульса — смещение bpm не сдвигают, в v1 игнорируются.

Тесты — `HrmParserTest` (UINT8/UINT16, 0xFF sign-extension, флаги, обрезанные payload'ы).

#### `data/hrm/sensor/HeartRateSensor.kt`
Транспорт-агностичный контракт источника пульса (аналог `LocationTracker`).
- `interface HeartRateSensor` — `startScan(onResult: (HrmScanResult) -> Unit, onError: (Int) -> Unit)`, `stopScan()`, `connect(address: String, listener: Listener)` (ОДНА попытка — политика ретраев в `HrmManager`), `disconnect()` (после вызова события не приходят, штатный разрыв не порождает `onDisconnected`).
- `interface Listener` — `onConnected()` (подписка на пульс активна), `onDisconnected(unexpected: Boolean)`, `onHeartRate(bpm: Int)`. Колбэки приходят на binder-потоках Bluetooth-стека.

#### `data/hrm/sensor/BleHeartRateSensor.kt`
BLE-реализация `HeartRateSensor` — единственный класс подсистемы, трогающий Android Bluetooth API.
- Скан: `BluetoothLeScanner.startScan` с `ScanFilter` по UUID сервиса 0x180D (в результаты попадают только пульсометры) + `SCAN_MODE_LOW_LATENCY`; `device.name` на API 31+ требует BLUETOOTH_CONNECT — при `SecurityException` имя берётся из `scanRecord.deviceName`.
- Подключение: `connectGatt(context, autoConnect=false, callback, TRANSPORT_LE)` → `discoverServices` → `setCharacteristicNotification(0x2A37, true)` + запись CCCD 0x2902 (API 33+ новый `writeDescriptor(desc, value)`, ниже — deprecated-путь); `onConnected` — только после успешной записи CCCD.
- Реализованы ОБА оверлоада `onCharacteristicChanged` (API 33 split — иначе нотификации молча не приходят на одной из сторон).
- Дисциплина против GATT 133 (нюанс 35): `close()` ВСЕГДА (и в `disconnect()`, и при любом обрыве в `failConnection`), `TRANSPORT_LE` явно, предыдущий gatt закрывается до нового `connect`. Все BT-вызовы в try/catch `SecurityException`.
- `companion object` — `HRS_SERVICE_UUID` (0x180D), `HR_MEASUREMENT_UUID` (0x2A37), `CCCD_UUID` (0x2902), `SCAN_ERROR_BLUETOOTH_OFF = -1` (свой код, не пересекается со `ScanCallback.SCAN_FAILED_*`).

#### `data/hrm/sensor/HeartRateSensorFactory.kt`
Фабрика сенсоров (паттерн `LocationTrackerFactory`).
- `HeartRateSensorFactory` (object) — `fun create(context: Context): HeartRateSensor = BleHeartRateSensor(context)`. KDoc фиксирует точку расширения ANT+: детектор доступности ANT Radio Service с guard'ом `NoClassDefFoundError` по образцу `RuntimeDetector`.

#### `data/hrm/HrmManager.kt`
Контракт менеджера пульсометра — единая точка владения соединением.
- `interface HrmManager` — `connectionState: StateFlow<HrmConnectionState>`, `currentSample: StateFlow<HrmSample?>` (null при обрыве — устаревший пульс не показывается), `scanResults: SharedFlow<HrmScanResult>` (дедупликация на подписчике), `isScanning: StateFlow<Boolean>`, `startScan()`, `stopScan()`, `connect(address: String)` (идемпотентен, останавливает скан), `disconnect()`, `fun freshBpm(maxAgeMs: Long = DEFAULT_FRESH_BPM_MAX_AGE_MS): Int?` (bpm только при CONNECTED и свежем сэмпле — для записи в GPS-точку).
- `companion object` — `DEFAULT_FRESH_BPM_MAX_AGE_MS = 10_000L`.

Особенности: @Singleton — экран «Датчики» и `LocationTrackingService` инжектят один экземпляр; соединение переживает уход с экрана и рвётся только явно (`disconnect`) или со смертью процесса.

#### `data/hrm/HrmManagerImpl.kt`
Реализация: state-machine соединения поверх «тонкого» сенсора.
- `HrmManagerImpl` (`@Singleton`, `@Inject constructor(@ApplicationContext Context)`).
- **Агрессивный реконнект без лимита попыток** (требование: тренировка максимально покрыта пульсом): одна попытка = `sensor.connect` + ожидание исхода с таймаутом `CONNECT_TIMEOUT_MS` (15 с); неудача → `sensor.disconnect()` (закрыть gatt — иначе 133) → пауза `RETRY_DELAY_MS` (1 с) → снова, бесконечно. Обрыв установленного соединения → немедленный перезапуск цикла (`RECONNECTING`). Реконнект по MAC без скана — троттлинг сканов Android (5/30 с) не задевает. Пассивного `autoConnect=true`-fallback НЕТ намеренно (ждёт десятки секунд).
- Потокобезопасность: все переходы состояния сериализуются через собственный scope c `limitedParallelism(1)`; исключение — `onHeartRate` пишет `currentSample.value` напрямую (атомарно, самое частое событие).
- `BroadcastReceiver` на `BluetoothAdapter.ACTION_STATE_CHANGED` (ленивая регистрация, NOT_EXPORTED): `STATE_OFF` → закрыть всё, `BLUETOOTH_OFF`, ретраи приостановлены; `STATE_ON` → немедленно возобновить. `internal fun onBluetoothOff()/onBluetoothOn()` — для тестов.
- Тестовые швы (internal var с боевыми дефолтами): `sensorProvider`, `timeSource`, `scope`, `bluetoothEnabledProvider`.
- `companion object` — `CONNECT_TIMEOUT_MS = 15_000L`, `RETRY_DELAY_MS = 1_000L`.

Тесты — `HrmManagerImplTest` (фейковый сенсор: реконнект без лимита, таймаут попытки, freshBpm-свежесть, BT off/on, скан).
...
**`@Binds` методы (все `@Singleton`):**
  ...
  - `bindAllowedEmailDomainsRepository(impl: AllowedEmailDomainsRepositoryImpl): AllowedEmailDomainsRepository` — 149-ФЗ, список российских почтовых доменов для регистрации; сетевая реализация (`GET /auth/allowed-email-domains`, BR-4) с кэшем память+DataStore, зашитый список — fallback.
  - `bindHrmManager(impl: HrmManagerImpl): HrmManager` — BLE-пульсометр; singleton владеет GATT-соединением, делится между экраном «Датчики» и `LocationTrackingService`.
...
**`@Provides` методы (в `companion object`, все `@Singleton` если не указано иное):**
  ...
  - `provideAllowedEmailDomainsDataStore(@ApplicationContext context): DataStore<Preferences>` — `@Named("allowedEmailDomains")`, отдаёт `context.allowedEmailDomainsDataStore` (делегат в файле репозитория); вынесен в провайдер, чтобы JVM-тесты подставляли DataStore на временном файле через `PreferenceDataStoreFactory` без Robolectric.
  - `provideOkHttpClient(tokenStorage: TokenStorage, tokenAuthenticator: TokenRefreshAuthenticator): OkHttpClient` — первым ставит User-Agent-интерцептор (`SmartTracker/<versionName> (<applicationId>; Android)` вместо дефолтного `okhttp/4.x` — требование OSMF Tile Usage Policy: через этот же клиент MapLibre качает OSM-тайлы, см. `SmartTrackerApp`/`OfflineMapManager`), затем auth-интерцептор через `buildAuthInterceptor(tokenStorage, apiHost)` из `data/remote/AuthInterceptor.kt` (`apiHost` извлекается из `BuildConfig.BASE_URL`; Bearer добавляется ТОЛЬКО на хост API — клиент общий с Coil/IconCacheManager, токен не должен уходить на внешние URL картинок), устанавливает `tokenAuthenticator` как `.authenticator(...)` (срабатывает на HTTP 401, обновляет токен через `/auth/refresh` и повторяет запрос; при повторном 401 — разлогинивает), добавляет `HttpLoggingInterceptor` с уровнем `BODY` в debug и `NONE` в release + `redactHeader("Authorization")` — токен не попадает даже в debug-logcat (тела auth-запросов при `BODY` всё равно логируются — logcat debug-сборки не передавать третьим лицам).
...
#### `data/repository/AllowedEmailDomainsRepositoryImplTest.kt`
Юнит-тесты цепочки источников BR-4 (7): нормализация trim+lowercase серверного списка; повторный вызов из памяти (api один раз); сбой сети без кэша → зашитый fallback; персистентность честно — второй экземпляр с упавшей сетью читает сохранённое первым (DataStore на временном файле через `PreferenceDataStoreFactory`, чистый JVM без Robolectric); пустой список от сервера = сбой → fallback; null вместо массива → fallback; неудача не мемоизируется (второй вызов снова дёргает api). Нюанс: сбой сети мокается `doAnswer { throw IOException() }` — suspend-сигнатура не декларирует checked-исключения, прямой `doThrow` не проходит валидацию Mockito.
...
- Особенность: используется современный API `LinkAnnotation.Clickable` вместо устаревшего `ClickableText` (см. п.10 CLAUDE.md). Содержит `@Preview` для обоих экранов (Terms/Privacy).

> **@Preview по presentation.** Экраны покрыты `@Preview`-функциями (private, в тех же файлах) по паттерну `LoginScreen`: `SmartTrackerTheme { Screen(state = UiState(...), колбэки = {}) }`, состояние — дефолтным конструктором с оверрайдами. Ключевые экраны — с вариантами (Register — 4 шага, WorkoutStart — pre-start/активная, Profile — загрузка/загружено). Контейнеры над VM (`ForgotPasswordScreen`, `TrainingHistoryScreen`) превьюятся через их stateless-составляющие (step-composable, timeline-views). Экраны с картой рендерятся благодаря preview-стабу `MapViewComposable` (`LocalInspectionMode`); composition-time системные вызовы, которые роняют layoutlib, гейтятся `LocalInspectionMode`: WorkoutStart (`BatteryOptimizationHelper`/PowerManager, `checkSelfPermission`, `LocationPermissionHandler`), Settings (AudioManager/TTS), Sensors (`BluetoothPermissionHandler`). Рендер проверяется в Android Studio (headless `@Preview` не рендерится — screenshot-тесты сознательно не добавлялись).
...
- `@Composable fun AppNavGraph(navController: NavHostController = rememberNavController(), startDestination: String = Screen.Login.route, onLogout: () -> Unit = {}, sessionExpired: StateFlow<Boolean> = MutableStateFlow(false))`.
  ...
  - `composable(Screen.Home.route)` — `WorkoutHomeScreen` с `onLogout` (вызывает `onLogout()` из параметров графа и очищает весь бэкстек `popUpTo(navController.graph.id) { inclusive = true }`), `onNavigateToProfile` и `onNavigateToSettings`. Датчики с HR-бейджа открываются оверлеем внутри WorkoutHomeScreen, не маршрутом (нюанс 36).
  - `composable(Screen.Settings.route)` — `SettingsViewModel` через `hiltViewModel()`, `state` через `collectAsStateWithLifecycle()`; `SettingsScreen` с `onBack = popBackStack`, четырьмя сеттер-колбэками ViewModel и `onOpenSensors = { navigate(Screen.Sensors.route) }`.
  - `composable(Screen.Sensors.route)` — `SensorsViewModel` через `hiltViewModel()`; `SensorsScreen` с `onBack = popBackStack` и колбэками ViewModel (разрешения, скан, `onSavedDeviceClick`/`onRemoveDeviceClick`/`onAddDeviceClick`).
...
- `SettingsViewModel` (`@HiltViewModel class SettingsViewModel(settingsStorage: SettingsStorage) : ViewModel()`).
  ...
  - `fun onAutopauseChanged(enabled: Boolean)` / `onVoiceCuesChanged(enabled: Boolean)` / `onVoiceCueIntervalChanged(intervalKm: Int)` / `onKeepScreenOnChanged(enabled: Boolean)` / `onFinishConfirmationHoldChanged(enabled: Boolean)` / `onShowHeartRateBadgeChanged(enabled: Boolean)` — fire-and-forget записи в storage (DataStore сериализует edit-транзакции; состояние обновится через подписку).
...
- `@Composable fun SettingsScreen(settings: AppSettings, onBack: () -> Unit, onAutopauseChanged, onVoiceCuesChanged, onVoiceCueIntervalChanged, onKeepScreenOnChanged, onFinishConfirmationHoldChanged, onShowHeartRateBadgeChanged, onOpenSensors: () -> Unit)` — `SwitchRow`-ы («Автопауза», «Голосовые подсказки», «Не гасить экран», «Удержание кнопки „Завершить"») + `IntervalSelectorRow` (частота подсказок 1/2/5 км) + `VolumeSliderRow` (громкость подсказок; оба видны только при включённых подсказках) + секция «Датчики»: `NavigationRow` «Пульсометр» (подзаголовок — имя/адрес активного датчика, иначе «Датчиков: N» при непустом списке, иначе «Не настроен»; клик → `onOpenSensors`) + `SwitchRow` «Бейдж пульса» (`showHeartRateBadge`).
- `@Composable private fun SwitchRow(title, subtitle, checked, onCheckedChange)` — заголовок + подпись + Material3 `Switch` в цветах темы.
- `@Composable private fun NavigationRow(title, subtitle, onClick)` — навигационный ряд (переход на вложенный экран): заголовок + текущее значение + стрелка «→»; стиль как `SwitchRow`.
...
#### `app/src/main/java/com/example/smarttracker/presentation/menu/sensors/BluetoothPermissionHandler.kt`
Запрос Bluetooth-разрешений для экрана «Датчики» (по образцу `LocationPermissionHandler`, но одношаговый).
- `@Composable fun BluetoothPermissionHandler(onGranted: () -> Unit, onDenied: () -> Unit)` — `LaunchedEffect(Unit)`: если все нужные разрешения выданы → `onGranted()`, иначе `rememberLauncherForActivityResult(RequestMultiplePermissions)`.
- `@Composable fun BluetoothEnablePrompt(onEnabled, onDismiss)` — `AlertDialog` «Bluetooth выключен» + «Включить»/«Отмена»; «Включить» запускает системный `ACTION_REQUEST_ENABLE` через launcher (RESULT_OK → `onEnabled`), при недоступности intent-а (нет BLUETOOTH_CONNECT / OEM) — fallback на `ACTION_BLUETOOTH_SETTINGS`.
- `private fun requiredBluetoothPermissions(): Array<String>` — API 31+: `BLUETOOTH_SCAN` + `BLUETOOTH_CONNECT`; API 26–30: `ACCESS_FINE_LOCATION` (BLE-скан до 31 требует геолокацию; обычно уже выдана для тренировок).

#### `app/src/main/java/com/example/smarttracker/presentation/menu/sensors/SensorsViewModel.kt`
ViewModel экрана/диалога «Датчики»: список сохранённых пульсометров, скан, добавление/удаление/переключение.
- `SensorsUiState` (data class) — `permissionsGranted`, `savedDevices: List<SavedHrmDevice>`, `activeAddress: String?`, `connectionState: HrmConnectionState`, `currentBpm: Int?` (только при CONNECTED), `isScanning`, `hasScanned`, `promptEnableBluetooth` (окно «включите Bluetooth»), `scanResults: List<HrmScanResult>` (дедуп по адресу, сортировка по rssi); computed `foundDevices` — `scanResults` без уже сохранённых (добавленный через «+» реактивно уезжает из «найденных»).
- `SensorsViewModel` (`@HiltViewModel`, инжектит `HrmManager` + `SettingsStorage`) — `init`: зеркалит `combine(settings, connectionState, currentSample, isScanning)` в UiState; отдельный коллектор накапливает `scanResults`.
  - `fun onPermissionsGranted()` — ставит флаг; BT выключен → `promptEnableBluetooth`, иначе `maybeAutoScan()`.
  - `fun onScanClick()` — ручной скан на тап (повторный тап во время скана игнорируется); BT выключен → `promptEnableBluetooth` вместо скана; иначе `startScan(SCAN_TIMEOUT_MS=30с)`.
  - `fun onDismissBluetoothPrompt()` / `fun onBluetoothEnabled()` — закрыть окно BT / после включения продолжить (`onScanClick`).
  - `private fun maybeAutoScan()` — автопоиск при входе БЕЗ сохранённых датчиков (список читается из `settings.first()`, не из стейта — collector мог не отработать), `startScan(AUTO_SCAN_TIMEOUT_MS=10с)`; после неудачи — вручную кнопкой.
  - `fun onAddDeviceClick(device: HrmScanResult)` — `addHrmDevice(address, name)` (сразу активный) + `connect`; persist немедленный (датчик только что найден сканом).
  - `fun onSavedDeviceClick(device: SavedHrmDevice)` — `setActiveHrmDevice` + `connect` (переключение).
  - `fun onRemoveDeviceClick(device: SavedHrmDevice)` — `removeHrmDevice`; если удаляется активный — `disconnect()`.
  - `fun onScanStopRequested()` — явная остановка скана + отмена таймаут-джобы; для закрытия `SensorsDialog` (VM скоупится на backstack-entry Home, переживает закрытие — onCleared не сработает).
  - `override fun onCleared()` — `stopScan()`; соединение НЕ рвётся намеренно (сервис пишет пульс с живого соединения; рвёт его в onDestroy).

Тесты — `SensorsViewModelTest`.

#### `app/src/main/java/com/example/smarttracker/presentation/menu/sensors/SensorsScreen.kt`
Экран «Датчики» (Настройки → Датчики) + общий контент для диалога. Скелет как `SettingsScreen` (Scaffold + CenterAlignedTopAppBar + `SmartTrackerBottomBar(MENU)`).
- `@Composable fun SensorsScreen(state, onBack, onPermissionsGranted, onPermissionsDenied, onScanClick, onSavedDeviceClick, onRemoveDeviceClick, onAddDeviceClick, onDismissBluetoothPrompt, onBluetoothEnabled)` — Scaffold-обвязка вокруг `SensorsScreenContent`.
- `@Composable fun SensorsScreenContent(state, onPermissionsGranted, onPermissionsDenied, onScanClick, onSavedDeviceClick, onRemoveDeviceClick, onAddDeviceClick, onDismissBluetoothPrompt, onBluetoothEnabled, modifier)` — минималистичный список: `BluetoothPermissionHandler`; `BluetoothEnablePrompt` при `promptEnableBluetooth`; секция сохранённых (`SavedDeviceRow` — точка статуса зелёная/янтарная/серая + имя + BPM у подключённого активного + корзина `ic_delete`; тап по строке = переключение) или подсказка; компактная pill-кнопка «Поиск» по центру (`ScanPillButton`); после скана — секция «Найденные» (`FoundDeviceRow` — имя + rssi + «+» `Icons.Filled.Add`) с `hasScanned`-гейтом и подсказкой при пустом результате. Хосты: `SensorsScreen` (маршрут) и `SensorsDialog` (диалог с экрана тренировки); размер задаёт хост через `modifier`, скролл внутри.
- Все строки — из `strings.xml` (`sensors_*`).

#### `app/src/main/java/com/example/smarttracker/presentation/menu/sensors/SensorsDialog.kt`
Компактный диалог «Датчики» поверх экрана тренировки (тап по HR-бейджу): карточка по центру со скримом. НЕ навигация и НЕ window-Dialog намеренно: navigate() с живой карты разрушает MapView и роняет процесс неотменяемыми аниматорами MapLibre (нюанс 36); тот же паттерн, что SummaryOverlay.
- `@Composable fun SensorsDialog(onClose: () -> Unit)` — `hiltViewModel<SensorsViewModel>()` (скоуп — backstack-entry Home) + `collectAsStateWithLifecycle`; `BackHandler(onClose)`; `DisposableEffect → onScanStopRequested()`; скрим `Box(fillMaxSize, Black 0.5α, clickable без indication → onClose)`; карточка `Column(fillMaxWidth(0.85f), White, RoundedCornerShape(20dp), pointerInput detectTapGestures глушит тапы, animateContentSize)` — заголовок + `SensorsScreenContent(heightIn max 440dp)`. Расширение вниз при появлении секции найденных — через `animateContentSize`. Хостится в `WorkoutHomeScreen` (Box поверх `WorkoutStartScreen`).
...
- `@Composable fun WorkoutHomeScreen(onLogout, onNavigateToProfile, onNavigateToSettings)` — хоистит `WorkoutStartViewModel` на верхнем уровне (не внутри `when`), чтобы оверлей итогов тренировки переживал переключение вкладок. Хостит оверлей «Датчики»: `showSensorsOverlay` (remember), ветка START обёрнута в Box — `SensorsOverlay` рисуется ПОВЕРХ `WorkoutStartScreen` (карта остаётся в композиции, нюанс 36); `onOpenSensors = { showSensorsOverlay = true }`; `LaunchedEffect(currentTab)` закрывает оба оверлея при смене вкладки. Использует `Scaffold` с `SmartTrackerBottomBar`; рендерит `WorkoutStartScreen`, `TrainingHistoryScreen` или `MenuScreen` в зависимости от выбранной вкладки.
...
- `@Composable fun MapViewComposable(modifier, currentLocation, lastKnownLocation, trackPoints, isTracking, isGpsActive, mapTilesFailed, onMapTilesFailed, enableLocationDot, locationPermissionGranted, suppressLocationDot, fitToTrackBoundsKey, scrubPoint, startIconRes, attributionTopEnd, recenterTrigger, snapshotRequest, onSnapshot)` — создаёт `MapView` через `AndroidView` (factory/update), подключает жизненный цикл через `DisposableEffect(lifecycleOwner)`, рисует слои трека (`LineLayer`), scrub-маркера (`CircleLayer`), стартовой/финишной точки (`SymbolLayer`), управляет `LocationComponent` (живая GPS-точка) и режимом камеры (`TRACKING`/`NONE`). `snapshotRequest: Int` + `onSnapshot: ((Bitmap) -> Unit)?` — счётчик-триггер снимка карты для шаринга (паттерн `recenterTrigger`): инкремент → `MapLibreMap.snapshot()` → Bitmap в колбэк; в кадр попадает только MapView, Compose-оверлеи — нет; карта не готова → запрос молча игнорируется. `suppressLocationDot: Boolean` — немедленно гасит LocationComponent (NONE → disable → cancelTransitions, как в ON_STOP); участвует в условии enable update-блока (`fitToTrackBoundsKey == null && !suppressLocationDot`) и ON_START-ветке — иначе они перевключают компонент на следующей же рекомпозиции. Оставлен как страховка: сам по себе краш навигации с живой карты НЕ предотвращает (уже запущенные AnimatorSet-ы неотменяемы) — навигация с живой карты запрещена вовсе, UI открывается оверлеями (нюанс 36).
- `private fun activateLocationComponent(map, style, context)` — активирует `LocationComponent` с кастомным drawable `ic_location_dot`, `accuracyAlpha=0`, `pulseEnabled=false`, `cameraMode=TRACKING`, `renderMode=NORMAL`. Внутри — runtime-проверка разрешения (Fine или Coarse через `ContextCompat.checkSelfPermission`): без разрешения — no-op (страховка последнего рубежа, см. нюанс 34 CLAUDE.md).
...
- **Preview-стаб** (`LocalInspectionMode.current`) — ранняя ветка в начале тела: в `@Preview` MapLibre не инициализирован и `MapView` во `AndroidView` не рендерится, поэтому вместо карты рисуется чистый `OfflineMapFallback` и функция выходит. Разблокирует превью экранов с картой (`WorkoutStartScreen`). Рантайм не затронут — `LocalInspectionMode` всегда `false` вне preview-хоста.
- **Разрешение геолокации гейтит LocationComponent** (`locationPermissionGranted`, дефолт true). Активация без разрешения роняет процесс `SecurityException`-ом изнутри MapLibre (`MapView.onStart` → `LocationComponent.onStart` → `getLastKnownLocation`) — вызов идёт из внутреннего Handler-а карты, внешние `runCatching` его не перехватывают (краш свежей установки). Активация пропускается в `setStyle`-колбэке пока разрешения нет; при смене false → true (юзер согласился в диалоге) — поздняя активация в `update`-блоке без пересоздания карты. `liveHead` трека тоже гейтится.
...
- `@Composable fun WorkoutStartScreen(state, padding, onStartClick, onTypeSelected, onSheetTypeSelected, onPauseClick, onFinishClick, onMapTilesFailed, onToggleFavorite, onSearchQueryChange, onCloseSummary, onToggleFullscreenMap, onDeleteHistoryTraining, onOpenSensors = {}, onCoachmarkDismissed = {})` — корневой Composable экрана. Строит `Column` с шапкой (дата + кнопка справки), опциональным Doze-баннером, телом (`AnimatedContent` по `overlayVisible`: `ActiveBody` ↔ `SummaryBody`; размерит блок по РЕАЛЬНОМУ контенту и плавно анимирует высоту при переходе — карта-сосед снизу с `weight(1f)` ресайзится, MapView не пересоздаётся; прежний alpha-Box держал `max(active,summary)` высоту и после выноса селектора оставлял пустоту под статами в активной фазе), картой `MapViewComposable` с наложенными GPS-бейджем, HR-бейджем (гейт `state.showHrBadge`, виден всегда как GPS — зелёный подключён/красный нет связи; тап → `onOpenSensors`) и кнопками, и прогресс-баром (только в fullscreen-оверлее). Также рендерит `ModalBottomSheet` выбора типа активности с поиском.
- **Pre-start-макет = карта-герой**: тело `ActiveBody` (таймер+статистика) показывается только когда `state.isWorkoutStarted || overlayVisible` — до старта блок схлопнут (`AnimatedVisibility`), карта занимает всю высоту от даты до низа. Низ карты — оверлей `BottomCenter`: в pre-start `Column { ActivityTypeSelector; Spacer(10dp); кнопка «Начать» }`; на паузе (`isWorkoutStarted && !isTracking`) и в активной фазе (`isTracking`) — один общий `PausableFinishRow` (лево «Продолжить»/onStartClick на паузе, «Пауза»/onPauseClick в активе; право — «Завершить»). С паузы теперь тоже можно завершить (эвристика Нильсена №3). Выбор активности живёт ТОЛЬКО в pre-start (тип не меняется в ходе записи — в активной фазе селектора нет).
...
- `@Composable private fun ActiveHeader(dateDisplay, onHelpClick)` — центрированная дата в шапке; в левом углу кнопка справки «?» (`R.drawable.ic_help`) → `onHelpClick` открывает онбординг-coachmark в любой момент (`coachmarkForced = true`). Хедер рендерится и в pre-start, и в активной фазе.
...
- `@Composable private fun WorkoutCoachmark(step, stepCount, finishConfirmationHold, finishAvailable, finishButtonBounds, gpsBadgeBounds, hrBadgeBounds, onNext, onBack, onDismiss)` + `CoachmarkTip(text)`, `GpsBadgeSample(active, label)`, `HrBadgeSample(connected, label)` — **многошаговый** (`COACHMARK_STEPS = 3`) onboarding. Показ: авто при первом входе в активную фазу (`isWorkoutStarted && !coachmarkShown`) ИЛИ принудительно по кнопке справки (`coachmarkForced`); гейт `!overlayVisible && !isFullscreen`. **Spotlight**: скрим `Black α0.62` с «дыркой» над контролом текущего шага — шаг 1 «Завершить» (углы как у кнопки: справа скруглены, слева квадратные), шаг 2 GPS-бейдж, шаг 3 HR-бейдж; `Canvas`+`Path` из `RoundRect`, `blendMode = BlendMode.Clear` в offscreen-`graphicsLayer`, обводка `ColorSecondary`, треугольник-стрелка (вниз к контролу снизу / вверх к контролу сверху по `targetAtTop`). Карточка позиционируется НАД/ПОД контролом; **если подсвечивать нечего** (`hole == null`, напр. справка из pre-start — кнопки «Завершить» нет, гейт `finishAvailable`) — по центру экрана. Контент: шаг 1 — советы (удержание/пауза/автопауза); шаг 2 — два `GpsBadgeSample` (Сигнал есть/Поиск сигнала) + центрирование по тапу + движение карты на паузе; шаг 3 — два `HrBadgeSample` (128 уд/мин / Нет связи) + тап→список датчиков + «можно скрыть в Настройках». Навигация: «Назад» (со 2-го шага, серая) / «Далее» / «Понятно» (на последнем). Bounds контролов hoisted (`finishButtonBounds`/`gpsBadgeBounds`/`hrBadgeBounds` из `onGloballyPositioned`/`boundsInWindow`). Если бейдж пульса выключен (`!showHrBadge`), на шаге 3 показывается демо-HR-бейдж на карте (копия на штатном месте) — чтобы было куда указывать. Тап по скриму / «Понятно» → сброс `coachmarkStep`/`coachmarkForced` + `onCoachmarkDismissed` (персист `setWorkoutCoachmarkShown`).
- `@Composable private fun HoldToFinishButton(onFinish, onShortTap, shape, modifier, demoFill)` — кнопка «Завершить» с подтверждением по удержанию 3 сек. `demoFill=true` (шаг 1 coachmark) → `LaunchedEffect`: стартовая пауза `delay(1500)`, затем цикл (snapTo 0 → animateTo 1 за 3с → delay 2с) — кнопка сама заполняется, демо механики удержания (не сразу — дать прочитать карточку). Дальше: `detectTapGestures.onPress` запускает `Animatable` 0→1 (`tween(3000, LinearEasing)`), заполнение `ColorSecondary` растёт слева направо (`fillMaxWidth(progress)` у Box с `align(CenterStart)`); дошло до 1 → `onFinish()`; отпускание раньше → `fill.cancel()` + откат к 0 (`tween(250)`). Обычный тап НЕ срабатывает намеренно. Рисуется вместо обычной `Button` при `state.finishConfirmationHold` (иначе — мгновенный тап).
...
- **Scrub-режим только в fullscreen.** `scrubPoint = if (isFullscreen) scrubPoint else null` — маркер scrubbing передаётся в карту только когда открыт полноэкранный режим; в компактном оверлее карта маленькая, маркер отвлекает. `ScrubDisplayStats.heartRateDisplay` заполняется из `trackPoints[scrubIndex].heartRate` (в `CumulativeTrackData` серии пульса нет), гейт — `summary.avgHeartRateDisplay != null`.
...
- **HR-бейдж** — под GPS-бейджем (top-end карты, `padding(top = 48.dp)`), виден всегда как GPS-бейдж (гейт `state.showHrBadge` — настройка, дефолт вкл; вне оверлея); статусный: круглый 32dp, иконка `Icons.Filled.Favorite`, фон `ColorGpsActive`/`ColorGpsInactive` по `hrmConnected` (зелёный подключён / красный нет связи, в т.ч. когда датчик не настроен). Значение пульса на бейдже НЕ показывается — оно в StatItem «Пульс» (дублирование на карте шумит). Тап → `onOpenSensors()` открывает компактный диалог `SensorsDialog` (хостится в WorkoutHomeScreen ПОВЕРХ WorkoutStartScreen) — НЕ навигацию: navigate() с живой карты разрушает MapView и роняет процесс гонкой неотменяемых аниматоров MapLibre со Style; тайминговые митигации не работают (нюанс 36). Лямбда приходит из `WorkoutHomeScreen` (`{ showSensorsOverlay = true }`).
...
- `data class UiState(...)` — большой стейт: `currentDate`, `workoutTypes`, `pinnedTypes`, `selectedType`, `isTracking`, `isWorkoutStarted`, `gpsStatus`, `elapsedMs`, `timerDisplay`, `distanceDisplay`/`distanceMeters`, `avgSpeedDisplay`, `caloriesDisplay`/`kilocalories`, `trackPoints`, `pauseGapIndices`, `mapTilesFailed`, `isStarting`, `keepScreenOn` (из настроек, флаг ставит экран), `hrmConfigured` (пульсометр настроен — гейт StatItem «Пульс» и автоконнекта), `showHrBadge` (настройка «Бейдж пульса», дефолт true — гейт HR-бейджа на карте, независим от датчика), `hrmConnected`, `heartRateDisplay` ("148"/"--"), `favoriteIds`, `searchQuery`, `lastKnownLocation`, `summaryOverlay: WorkoutSummaryUiState?`, `isMapFullscreen`. Вычисляемые свойства: `isGpsActive`, `filteredAndSortedTypes`. Конструктор дополнительно принимает `SettingsStorage` (подписка на `keepScreenOn`/`hrmConfigured=hrmDevices.isNotEmpty()`/`showHrBadge=showHeartRateBadge` в `init`; там же — **автоподключение активного пульсометра при входе на экран**: `autoConnectAddress()` + разрешение BLUETOOTH_CONNECT (`hasBluetoothConnectPermission`, ниже API 31 — true) → `hrmManager.connect(address)`, идемпотентно, не дожидаясь старта тренировки) и `HrmManager` (живой пульс: `combine(connectionState, currentSample)` → `hrmConnected`/`heartRateDisplay`; напрямую из менеджера, не через Room — GPS-точки редкие, пульс тикает ~1/с).
...
- `fun showHistorySummary(item: TrainingHistoryItem, activityName: String)` — показывает оверлей итогов для тренировки из истории: сразу `isLoading=true`, затем асинхронно тянет GPS-трек через `GET /training/{id}/get_training`. Gap-индексы пауз восстанавливаются эвристикой `SplitsBuilder.detectPauseGapIndices(points)` (сервер их не хранит) и идут в `buildCumulativeData` + snapshot (`pauseGapIndices`) — elapsed в scrub, сплиты, сегментация графика и GPX-сегменты учитывают паузы. Пульс: приоритет — серверные агрегаты `item.avgHeartRate`/`item.maxHeartRate` (BR-16; доступны даже если трек не загрузился — паттерн elevation), fallback — расчёт по `points.mapNotNull { heartRate }` (тренировки с пульсом в точках, но без агрегатов); обе ветки null → поля null, секция скрыта.
...
- `private fun observeTrackingData(trainingId)` — единый наблюдатель GPS-точек: на каждой эмиссии ПОЛНЫЙ пересчёт статистики через `buildCumulativeData(points, pauseGapIndices)` (тот же путь, что оверлей/scrub — единый источник истины) на `Dispatchers.Default`; дистанция = последнее `distancesKm`, темп = дистанция / последнее `elapsedMs` (уже без пауз), калории = `points.sumOf { calories }`; GPS-таймаут (`GPS_FIX_TIMEOUT_MS`) без новых точек → `UNAVAILABLE` (тренировка не останавливается).
...
Особенности/нюансы (ключевые):
- **Live-статистика = полный пересчёт через `buildCumulativeData`, НЕ инкрементально** (нюанс 37). Прежний инкрементальный аккумулятор не мог ретроактивно вычесть «телепорт» через паузу, если gap-индекс приходил ПОСЛЕ обработки его пост-резюм точки (async-гонка `recordingStateFlow`→state против Room-потока, флаппинг автопаузы) — телепорт впечатывался в live-дистанцию навсегда, live расходился с scrub, кривое значение сохранялось. Полный пересчёт самокорректируется (gap на следующей эмиссии уже в state) и даёт live == scrub == сохраняемое. O(n) на эмиссию, для реального числа точек незначимо; re-key `localUUID → serverUUID` больше не требует защиты от задвоения (пересчёт с нуля).
- **Gap-индексы паузы** (`pauseGapIndices`) — точный индекс первой пост-резюм точки приходит от сервиса (`LocationTrackingService.recordingStateFlow`), а не вычисляется по размеру `trackPoints` (тот отстаёт от буфера сервиса на 1-2 точки). `buildCumulativeData` пропускает "телепорт"-пары и вычитает время паузы из elapsed. Для оверлея истории gap'ы восстанавливаются эвристикой `SplitsBuilder.detectPauseGapIndices` (сервер их не хранит) и кладутся в snapshot (`cumulativeData`, график, GPX-сегменты).
...
- **Реальные значения дистанции/калорий шлются на сервер** (`total_distance_meters`/`total_kilocalories`) — сервер их и хранит (BR-18: клиентская дистанция приоритетна, т.к. только клиент знает о паузах). Воркэраунд `?: 0.0` снят (BR-7 закрыт бэком); null допустим для legacy-записей очереди.
...
- `data class ScrubDisplayStats(speedDisplay, elapsedDisplay, distanceDisplay, elevationDisplay, heartRateDisplay: String? = null)` — данные для отображения в момент scrubbing, заменяют суммарные значения; `heartRateDisplay` — пульс в точке scrub ("—" при локальном пропуске сэмпла; null = у тренировки нет данных пульса — строка скрыта).
- `@Composable fun StatsOverlayCard(state, modifier, scrubStats)` — компактная карточка 133×100dp (122dp при наличии пульса) из 4–5 строк статистики поверх карты в fullscreen-режиме (Figma 723:460); если передан `scrubStats`, отображает значения в текущей точке скраба вместо итоговых; строка пульса: при scrubbing — из точки, без — `avgHeartRateDisplay`.
- `@Composable private fun StatsOverlayRow(iconRes, value)` — одна строка карточки (иконка + значение).
- `@Composable private fun StatsOverlayHeartRow(value)` — строка пульса (векторная `Icons.Filled.Favorite` — drawable-иконки сердца в проекте нет).
...
- `fun formatHeartRate(bpm: Int): String` — "148 уд/мин" (avg/max пульс в панели деталей итогов).
...
- `data class WorkoutSummaryUiState(origin, trainingId, dateDisplay, activityName, activityIconFile, activityIconUrl, activityIconKey, paceDisplay, distanceDisplay, durationDisplay, elevationDisplay, trackPoints, cumulativeData, splits, pauseGapIndices, avgHeartRateDisplay, maxHeartRateDisplay, isLoading)` — итоговый снимок, все строковые поля уже отформатированы на момент построения. `splits: List<SplitUi>` — километровые сплиты для панели деталей (пусто без настоящих временных меток); `pauseGapIndices` — для разрывов графика скорости (для HISTORY всегда пуст); `avgHeartRateDisplay`/`maxHeartRateDisplay: String?` — "148 уд/мин" или null (в точках нет пульса: датчик не подключался, история до BR-16) — секция «Пульс» в панели деталей скрыта.
...
- `SplitsBuilder` (object):
  ...
  - `const MIN_PAUSE_GAP_MS = 15_000L` — минимальный разрыв времени, считающийся паузой (эвристика истории).
  ...
  - `fun detectPauseGapIndices(points: List<LocationPoint>): List<Int>` — эвристика границ пауз для истории (сервер gap-индексы не хранит, но на паузе точек нет → пауза видна как разрыв времени): gap там, где интервал > max(15 c, 3 × медианный интервал трека); медиана устойчива к самим паузам, порог 15 с страхует редкую запись. Возвращает индексы первой пост-резюм точки (формат `pauseGapIndices`). Синтетические таймстемпы (интервалы ~1 мс) порог не пробивают — самогейт, `hasRealTiming` не нужен. Используется в `showHistorySummary` (нюанс 37).
...
Чистые JUnit-тесты `SplitsBuilder` (14): равномерный трек → равные сплиты с одинаковым темпом; интерполяция границы между точками; неполный хвост с фактическим темпом; хвост < 50 м отбрасывается; трек < 1 км → один partial-сплит; relativeSpeed нормируется на самый быстрый круг; синтетические таймстемпы истории гейтятся (`hasRealTiming`); вырожденные данные не падают; buildSplits доверяет elapsed (паузы уже вычтены). `detectPauseGapIndices` (5): равномерный трек → пусто; разрыв 60с на фоне 5с-интервалов → индекс первой пост-паузной точки; синтетические таймстемпы → пусто; < 3 точек → пусто; редкая, но равномерная запись (30с) не принимается за паузы (медианный фактор).
...
Дистанция/темп и паузы (нюанс 37): recovery с gap-индексом + точки с
«телепортом» → дистанция исключает gap-пару, длительность темпа — её время
(`pausedNanos`); ожидание реального `Dispatchers.Default` — полл с
`advanceUntilIdle` (виртуальное время runTest его не покрывает);
`createViewModel(points = ...)` подставляет трек в
`observePointsForTraining`.
