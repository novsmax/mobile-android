...
по состоянию на 2026-07-13, коммит `2d9d510`, ветка `claude/smarttracker-ant-plus-hrm-18d856`.
...
- `AppSettings` (data class) — снимок настроек с осознанными дефолтами:
  ...
  - `hrmDeviceAddress: String? = null` — MAC сохранённого BLE-пульсометра (null = не настроен; отдельного toggle нет: адрес сохранён = включено);
  - `hrmDeviceName: String? = null` — имя пульсометра для отображения в UI.
...
- `SettingsStorage` (interface):
  ...
  - `suspend fun setHrmDevice(address: String?, name: String?)` — сохранить пульсометр; `address = null` стирает оба поля («Забыть датчик»).

Потребители: `SettingsViewModel` (чтение+запись), `LocationTrackingService` (автопауза, TTS, адрес пульсометра для автоподключения), `WorkoutStartViewModel` (keepScreenOn, hrmConfigured), `SensorsViewModel` (чтение+запись пульсометра).
...
- `SettingsStorageImpl` (`@Singleton`, `@Inject constructor(@ApplicationContext Context)`):
  - `Keys` (private object) — `booleanPreferencesKey`/`intPreferencesKey` для четырёх настроек + `stringPreferencesKey` `HRM_DEVICE_ADDRESS`/`HRM_DEVICE_NAME`;
  ...
  - сеттеры — `context.settingsDataStore.edit { … }`; `setVoiceCueIntervalKm` валидирует значение; `setHrmDevice(null, …)` делает `remove()` обоих HRM-ключей.
...
- `LocationTrackingService` (`@AndroidEntryPoint`, class, наследует `Service`) — инжектируемые поля: `LocationRepository`, `WorkoutRepository`, `OfflineMapManager`, `SettingsStorage`, `HrmManager` (пульсометр: автоподключение, пульс в точки, разрыв в onDestroy).
  ...
  - `private fun connectHrmIfConfigured()` — автоподключение к сохранённому пульсометру: адрес из `settingsStorage.settings.first().hrmDeviceAddress`, гейт `hasBluetoothConnectPermission()`; вызывается из ветки `EXTRA_TRANSITION_TO_WORKOUT`, обычного старта тренировки и START_STICKY-рестарта; в discovery НЕ вызывается. Дальше `HrmManager` сам удерживает соединение.
...
- `@Composable fun AppNavGraph(navController: NavHostController = rememberNavController(), startDestination: String = Screen.Login.route, onLogout: () -> Unit = {}, sessionExpired: StateFlow<Boolean> = MutableStateFlow(false))`.
  ...
  - `composable(Screen.Sensors.route)` — `SensorsViewModel` через `hiltViewModel()`; `SensorsScreen` с `onBack = popBackStack` и колбэками ViewModel (разрешения, скан, подключение, «Забыть датчик»).
...
- `SensorsUiState` (data class) — `permissionsGranted`, `savedDeviceAddress/savedDeviceName: String?`, `connectionState: HrmConnectionState`, `currentBpm: Int?` (только при CONNECTED), `isScanning`, `hasScanned` (гейт подсказки «не найдено» — до первого скана не показывается), `scanResults: List<HrmScanResult>` (дедуп по адресу, сортировка по rssi).
- `SensorsViewModel` (`@HiltViewModel`, инжектит `HrmManager` + `SettingsStorage`) — `init`: зеркалит `combine(settings, connectionState, currentSample, isScanning)` в UiState; отдельный коллектор накапливает `scanResults`; третий коллектор персистит выбранное устройство `setHrmDevice(...)` ТОЛЬКО после первого `CONNECTED` (клик по недоступному датчику не затирает рабочий сохранённый).
  - `fun onScanClick()` — один скан на тап (повторный тап во время скана игнорируется), очистка списка, автостоп через `SCAN_TIMEOUT_MS` (30 с — троттлинг Android >4 сканов/30 с).
  - `fun onDeviceClick(device: HrmScanResult)` — `pendingSave = device`, `hrmManager.connect(...)`.
  - `fun onConnectSavedClick()` — подключение к сохранённому датчику (проверка связи вне тренировки).
  - `fun onForgetClick()` — `disconnect()` + `setHrmDevice(null, null)`.
  - `fun onScanStopRequested()` — явная остановка скана + отмена таймаут-джобы; для закрытия `SensorsOverlay` (VM скоупится на backstack-entry Home, переживает закрытие оверлея — onCleared не сработает).
  - `override fun onCleared()` — `stopScan()`; соединение НЕ рвётся намеренно: пользователь подключил датчик → пошёл на тренировку, сервис пишет пульс с уже живого соединения (рвёт сервис в onDestroy).

Тесты — `SensorsViewModelTest`.

#### `app/src/main/java/com/example/smarttracker/presentation/menu/sensors/SensorsScreen.kt`
Экран «Датчики» (Настройки → Датчики). Скелет как `SettingsScreen` (Scaffold + CenterAlignedTopAppBar + `SmartTrackerBottomBar(MENU)`).
- `@Composable fun SensorsScreen(state: SensorsUiState, onBack, onPermissionsGranted, onPermissionsDenied, onScanClick, onDeviceClick, onConnectSavedClick, onForgetClick)` — Scaffold-обвязка вокруг `SensorsScreenContent`.
- `@Composable fun SensorsScreenContent(state, onPermissionsGranted, onPermissionsDenied, onScanClick, onDeviceClick, onConnectSavedClick, onForgetClick, modifier)` — содержимое без Scaffold: `BluetoothPermissionHandler`; карточка сохранённого датчика (`SavedDeviceCard`: имя, статус + живой BPM при CONNECTED, кнопки «Подключить»/«Забыть датчик») или подсказка «датчик не настроен»; `PrimaryButton` «Поиск датчиков» (disabled без разрешений/во время скана, спиннер при скане); список `ScanResultRow` (имя/адрес/rssi, клик → подключение); подсказки при отказе в разрешениях и пустом результате скана (`hasScanned`-гейт). Используется двумя хостами: `SensorsScreen` (маршрут) и `SensorsOverlay` (оверлей с экрана тренировки).
- Все строки — из `strings.xml` (`sensors_*`).

#### `app/src/main/java/com/example/smarttracker/presentation/menu/sensors/SensorsOverlay.kt`
Оверлей «Датчики» поверх экрана тренировки (тап по HR-бейджу). НЕ навигация намеренно: navigate() с живой карты разрушает MapView и роняет процесс неотменяемыми аниматорами MapLibre (нюанс 36); тот же паттерн, что SummaryOverlay.
- `@Composable fun SensorsOverlay(onClose: () -> Unit)` — `hiltViewModel<SensorsViewModel>()` (скоуп — backstack-entry Home, переживает закрытие) + `collectAsStateWithLifecycle`; `BackHandler(onClose)` — системный Back закрывает оверлей, а не экран; `DisposableEffect → onDispose → viewModel.onScanStopRequested()` — скан не живёт после закрытия (onCleared не сработает); белый фон `fillMaxSize` + `statusBarsPadding` + шапка (стрелка «←» + заголовок «Датчики») + `SensorsScreenContent`. Хостится в `WorkoutHomeScreen` (Box поверх `WorkoutStartScreen`).
...
- **HR-бейдж** — под GPS-бейджем (top-end карты, `padding(top = 48.dp)`), виден только при `hrmConfigured` и вне оверлея; статусный: круглый 32dp, иконка `Icons.Filled.Favorite`, фон `ColorGpsActive`/`ColorGpsInactive` по `hrmConnected`. Значение пульса на бейдже НЕ показывается — оно в StatItem «Пульс» (дублирование на карте шумит). Тап → `onOpenSensors()` открывает **оверлей** `SensorsOverlay` (хостится в WorkoutHomeScreen ПОВЕРХ WorkoutStartScreen) — НЕ навигацию: navigate() с живой карты разрушает MapView и роняет процесс гонкой неотменяемых аниматоров MapLibre со Style; тайминговые митигации не работают (нюанс 36). Лямбда приходит из `WorkoutHomeScreen` (`{ showSensorsOverlay = true }`).
