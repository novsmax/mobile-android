...
по состоянию на 2026-07-22, коммит `9400792`, ветка `main`.
...
- `SettingsViewModel` (`@HiltViewModel class SettingsViewModel(settingsStorage: SettingsStorage) : ViewModel()`).
  ...
  - `fun onAutopauseChanged(enabled: Boolean)` / `onVoiceCuesChanged(enabled: Boolean)` / `onVoiceCueIntervalChanged(intervalKm: Int)` / `onKeepScreenOnChanged(enabled: Boolean)` / `onFinishConfirmationHoldChanged(enabled: Boolean)` — fire-and-forget записи в storage (DataStore сериализует edit-транзакции; состояние обновится через подписку).
...
- `@Composable fun SettingsScreen(settings: AppSettings, onBack: () -> Unit, onAutopauseChanged, onVoiceCuesChanged, onVoiceCueIntervalChanged, onKeepScreenOnChanged, onFinishConfirmationHoldChanged, onOpenSensors: () -> Unit)` — `SwitchRow`-ы («Автопауза», «Голосовые подсказки», «Не гасить экран», «Удержание кнопки „Завершить"») + `IntervalSelectorRow` (частота подсказок 1/2/5 км) + `VolumeSliderRow` (громкость подсказок; оба видны только при включённых подсказках) + секция «Датчики» с `NavigationRow` «Пульсометр» (подзаголовок — имя/адрес активного датчика, иначе «Датчиков: N» при непустом списке, иначе «Не настроен»; клик → `onOpenSensors`).
...
- `@Composable private fun WorkoutCoachmark(finishConfirmationHold, finishButtonBounds, onDismiss)` + `CoachmarkTip(text)` — одноразовый onboarding при первом входе в активную фазу (`isWorkoutStarted && !coachmarkShown && !overlayVisible && !isFullscreen`). **Spotlight**: скрим `Black α0.62` с «дыркой» точно над кнопкой «Завершить» — она светится сквозь затемнение (`Canvas` + `Path` из `RoundRect` с `blendMode = BlendMode.Clear` в offscreen-слое `graphicsLayer{ compositingStrategy = Offscreen }`; углы как у кнопки — справа скруглены, слева квадратные), обведена `ColorSecondary`, к ней ведёт треугольник-стрелка; карточка советов (удержание Завершить, кнопки на паузе, автопауза в настройках) + «Понятно» позиционируется НАД кнопкой. Bounds «Завершить» приходят из `PausableFinishRow` (`onGloballyPositioned`/`boundsInWindow`) → hoisted `finishButtonBounds`; оверлей меряет свою `positionInWindow`/`size` для перевода в локальные координаты. Тап по скриму или «Понятно» → `onCoachmarkDismissed` (персист `setWorkoutCoachmarkShown`). Корневой `Column` экрана обёрнут в `Box(fillMaxSize)` для оверлея.
- `@Composable private fun HoldToFinishButton(onFinish, onShortTap, shape, modifier, demoFill)` — кнопка «Завершить» с подтверждением по удержанию 3 сек. `demoFill=true` (пока показан coachmark) → `LaunchedEffect` гоняет `progress` по циклу (snapTo 0 → animateTo 1 за 3с → delay 2с), кнопка сама заполняется — демо механики удержания. Дальше: `detectTapGestures.onPress` запускает `Animatable` 0→1 (`tween(3000, LinearEasing)`), заполнение `ColorSecondary` растёт слева направо (`fillMaxWidth(progress)` у Box с `align(CenterStart)`); дошло до 1 → `onFinish()`; отпускание раньше → `fill.cancel()` + откат к 0 (`tween(250)`). Обычный тап НЕ срабатывает намеренно. Рисуется вместо обычной `Button` при `state.finishConfirmationHold` (иначе — мгновенный тап).
...
- **HR-бейдж** — под GPS-бейджем (top-end карты, `padding(top = 48.dp)`), виден только при `hrmConfigured` и вне оверлея; статусный: круглый 32dp, иконка `Icons.Filled.Favorite`, фон `ColorGpsActive`/`ColorGpsInactive` по `hrmConnected`. Значение пульса на бейдже НЕ показывается — оно в StatItem «Пульс» (дублирование на карте шумит). Тап → `onOpenSensors()` открывает компактный диалог `SensorsDialog` (хостится в WorkoutHomeScreen ПОВЕРХ WorkoutStartScreen) — НЕ навигацию: navigate() с живой карты разрушает MapView и роняет процесс гонкой неотменяемых аниматоров MapLibre со Style; тайминговые митигации не работают (нюанс 36). Лямбда приходит из `WorkoutHomeScreen` (`{ showSensorsOverlay = true }`).
