...
по состоянию на 2026-07-04, коммит `400ac91`, ветка `claude/friendly-stonebraker-22c8be`.
...
**Версия приложения:** `0.2` (versionCode 1), `applicationId = com.example.smarttracker`.
...
| Параметр | Значение |
|---|---|
| `namespace` / `applicationId` | `com.example.smarttracker` |
...
Security Crypto 1.1.0-alpha06, DataStore Preferences 1.1.1, Robolectric 4.13.
...
- `app/proguard-rules.pro` — правила ProGuard (минификация отключена в release: `isMinifyEnabled = false`).
...
- `LocationConfig` (object) — не имеет функций, только константы.
  ...
  - Crash-recovery ключи SharedPreferences: `PREFS_RECOVERY`, `KEY_ACTIVE_TRAINING`, `KEY_SESSION_STARTED_AT`, `KEY_PAUSED_ACCUMULATED_MS`, `KEY_RECORDED_POINT_COUNT`, `KEY_IS_RECORDING` (пауза переживает рестарт), `KEY_TRAINING_STARTED_AT`, `KEY_LAST_PERSIST_AT` (heartbeat), `KEY_PAUSE_GAP_INDICES`, `KEY_IS_REGISTERED`, профиль калорий/трекинга (`KEY_TYPE_ACTIV_ID`, `KEY_WEIGHT_KG`, `KEY_HEIGHT_CM`, `KEY_AGE_YEARS`, `KEY_IS_MALE`, `KEY_INTERVAL_MS`, `KEY_ACCURACY_THRESHOLD`), `RECOVERY_STALE_MS=2 мин` (порог протухания сессии).
...
- `LocationTrackingService` (`@AndroidEntryPoint`, class, наследует `Service`) — инжектируемые поля: `LocationRepository`, `WorkoutRepository`, `OfflineMapManager`.
  ...
  - `override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int` — центральный диспетчер команд через `Intent.hasExtra`, ветвление по типу команды:
    ...
    - `EXTRA_RECORDING` — команда паузы/резюма (в т.ч. из notification): пересчитывает `sessionStartedAt`/`pausedAccumulatedMs` для chronometer, эмитит `_recordingStateFlow`.
  ...
  - `private fun buildNotification(): Notification` — rich-уведомление с chronometer (только во время записи), текущей скоростью в км/ч, кнопкой Пауза/Продолжить, тапом на MainActivity.
  - `private fun rebuildNotification()` — перерисовка уведомления (no-op в discovery-режиме).
  - `private fun persistSessionState()` — сохраняет chronometer-состояние в `recoveryPrefs`.
...
Класс `Application`, точка входа Hilt-графа зависимостей и место разовой инициализации сторонних библиотек (MapLibre, Coil).
- `SmartTrackerApp` (`@HiltAndroidApp class SmartTrackerApp : Application(), Configuration.Provider, ImageLoaderFactory`) — инжектирует `HiltWorkerFactory` и `OkHttpClient` через `@Inject lateinit var`.
  - `fun onCreate(): Unit` — инициализирует `MapLibre.getInstance(...)` (без API-ключа, стандартный tile-сервер) и подменяет нативный HTTP-стек MapLibre на `okHttpClient` через `HttpRequestUtil.setOkHttpClient` — нужно, чтобы MapLibre доверял системному Android trust store, а не собственному TLS-bundle.
...
- `@Composable fun TermsOfServiceScreen(onBack: () -> Unit = {})` — `Scaffold` с `TopAppBar` (заголовок «Условия использования», кнопка назад), прокручиваемый `Text` с полным текстом соглашения (эффективная дата «18 марта 2026 г.», контакт `support@smarttracker.com`).
- `@Composable fun PrivacyPolicyScreen(onBack: () -> Unit = {})` — аналогичная структура, текст политики конфиденциальности (контакт `privacy@smarttracker.com`).
- Особенность: текст обоих экранов — статичные многострочные raw-строки (`"""..."""`), не вынесены в строковые ресурсы.
...
- Цвета: `ColorPrimary` (тёмно-синий `#0A1928`), `ColorSecondary` (мятно-бирюзовый `#4DACA7`), `ColorPlaceholder`, `ColorLink`, `ColorBackground`/`ColorWhite` (белый), `ColorDestructive` (красный `#FC3F1D` — для удаления/ошибок GPS), `ColorGpsActive`/`ColorGpsInactive` (workout-специфичные), `ColorFieldFill` (серый).
...
- `@Composable fun AppNavGraph(navController: NavHostController = rememberNavController(), startDestination: String = Screen.Login.route, onLogout: () -> Unit = {}, sessionExpired: StateFlow<Boolean> = MutableStateFlow(false))`.
  ...
  - `composable(Screen.Home.route)` — `WorkoutHomeScreen` с `onLogout` (вызывает `onLogout()` из параметров графа и очищает весь бэкстек `popUpTo(navController.graph.id) { inclusive = true }`) и `onNavigateToProfile`.
...
Вкладка «Меню»: навигационная сетка разделов приложения + демо-лента достижений. Для MVP рабочая ссылка — только «Профиль».
- `@Composable fun MenuScreen(padding: PaddingValues, onNavigateToProfile: () -> Unit)` — `Column` со скроллом, включает `MenuHeader()`, `MenuGrid(...)`, `AchievementsSection()`.
...
- `@Composable private fun MenuGrid(onNavigateToProfile: () -> Unit)` — 5 пунктов («Клубы», «Достижения», «Шаблоны», «Профиль», «Настройки»); только «Профиль» имеет реальный `onClick` (`onNavigateToProfile`), остальные — `noOp`; раскладка: первый ряд — 4 элемента, второй ряд — 1 элемент прижат влево.
...
- `@Composable fun WorkoutHomeScreen(onLogout, onNavigateToProfile)` — хоистит `WorkoutStartViewModel` на верхнем уровне (не внутри `when`), чтобы оверлей итогов тренировки переживал переключение вкладок. Использует `Scaffold` с `SmartTrackerBottomBar`; рендерит `WorkoutStartScreen`, `TrainingHistoryScreen` или `MenuScreen` в зависимости от выбранной вкладки.
...
Трёхшаговый Composable-обработчик системных разрешений, необходимых для GPS-трекинга тренировок.
...
- **BackHandler**: в fullscreen сворачивает карту к обычному оверлею, иначе закрывает оверлей целиком.
...
- `data class UiState(...)` — большой стейт: `currentDate`, `workoutTypes`, `pinnedTypes`, `selectedType`, `isTracking`, `isWorkoutStarted`, `gpsStatus`, `elapsedMs`, `timerDisplay`, `distanceDisplay`/`distanceMeters`, `avgSpeedDisplay`, `caloriesDisplay`/`kilocalories`, `trackPoints`, `pauseGapIndices`, `mapTilesFailed`, `isStarting`, `favoriteIds`, `searchQuery`, `lastKnownLocation`, `summaryOverlay: WorkoutSummaryUiState?`, `isMapFullscreen`. Вычисляемые свойства: `isGpsActive`, `filteredAndSortedTypes`.
...
- `@Composable fun SummaryHeader(dateDisplay, showDelete, onDeleteClick)` — шапка с центрированной датой; иконка корзины (с `AlertDialog` подтверждения "Удалить тренировку?") показывается только при `showDelete=true` (только для `SummaryOrigin.HISTORY`).
- `@Composable fun SummaryBody(state: WorkoutSummaryUiState)` — обёртка: `ActivityHeader` + `StatsRow`.
- `@Composable private fun ActivityHeader(state)` — иконка активности (74dp рамка) + название + разделитель + темп.
- `@Composable private fun StatsRow(state)` — три `StatCard` (дистанция/продолжительность/набор высоты) + декоративная стрелка-чеврон поверх правой границы третьей карточки.
...
- `data class WorkoutSummaryUiState(origin, trainingId, dateDisplay, activityName, activityIconFile, activityIconUrl, activityIconKey, paceDisplay, distanceDisplay, durationDisplay, elevationDisplay, trackPoints, cumulativeData, isLoading)` — итоговый снимок, все строковые поля уже отформатированы на момент построения.
