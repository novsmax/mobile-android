...
- **HR-бейдж** — под GPS-бейджем (top-end карты, `padding(top = 48.dp)`), виден только при `hrmConfigured` и вне оверлея; статусный: круглый 32dp, иконка `Icons.Filled.Favorite`, фон `ColorGpsActive`/`ColorGpsInactive` по `hrmConnected`. Значение пульса на бейдже НЕ показывается — оно в StatItem «Пульс» (дублирование на карте шумит). Тап → `onOpenSensors` (экран «Датчики»: быстрый путь к переподключению датчика мид-тренировки); лямбда пробрасывается `AppNavGraph` → `WorkoutHomeScreen` → `WorkoutStartScreen`.
