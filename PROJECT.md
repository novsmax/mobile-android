...
по состоянию на 2026-07-21, коммит `1dda6cd`, ветка `main`.
...
- **Gap-индексы паузы** (`pauseGapIndices`) — точный индекс первой пост-резюм точки приходит от сервиса (`LocationTrackingService.recordingStateFlow`), а не вычисляется по размеру `trackPoints` (тот отстаёт от буфера сервиса на 1-2 точки). Используются и в live-расчёте, и в `buildCumulativeData` — обе пропускают "телепорт"-пары через паузу; live-расчёт дополнительно копит их время в `pausedNanos` и вычитает из длительности среднего темпа (иначе темп проседал после каждой паузы — раньше было «известным ограничением»). Для оверлея истории gap'ы восстанавливаются эвристикой `SplitsBuilder.detectPauseGapIndices` (сервер их не хранит) и кладутся в snapshot (`cumulativeData`, график, GPX-сегменты).
